...
**Provides**: Structured development workflow skills for test-driven development, systematic debugging, collaborative planning, and multi-agent execution.

**Key skills:**
| Skill | Invoke | Purpose |
|-------|--------|---------|
| Test-Driven Development | `/test-driven-development` | RED-GREEN-REFACTOR cycle with anti-pattern detection |
| Systematic Debugging | `/systematic-debugging` | 4-phase root-cause analysis (tracing, defense-in-depth, conditions, waiting) |
| Brainstorming | `/brainstorming` | Socratic design refinement via guided questions before coding |
| Writing Plans | `/writing-plans` | Breaks large features into 2–5 min bite-sized tasks |
| Executing Plans | `/executing-plans` | Batch execution with human checkpoints |
| Subagent-Driven Dev | `/subagent-driven-development` | Two-stage review: spec compliance → code quality |
| Git Worktrees | `/using-git-worktrees` | Parallel development branches without conflicts |
| Verification | `/verification-before-completion` | Confirms fixes actually resolve the issue |

**Use when**:
- Adding new cards → `/brainstorming` to validate design, then `/test-driven-development` for implementation
- Debugging card issues → `/systematic-debugging` instead of trial-and-error guessing
- Large refactors or multi-card batches → `/writing-plans` + `/executing-plans` to parallelize safely
- Parallel feature development → `/using-git-worktrees` for isolated branches
- After bug fixes → `/verification-before-completion` to confirm resolution
...
4. **Debugging card rendering** → `/systematic-debugging` for root-cause analysis, then console.warn logging + browser devtools
5. **User says "use the HA skill"** → Invoke `/home-assistant-best-practices`
6. **Adding a new card** → `/brainstorming` to validate design → `/test-driven-development` for RED-GREEN-REFACTOR → `/verification-before-completion` after done
7. **Large multi-card feature or refactor** → `/writing-plans` to break into tasks → `/executing-plans` with subagents → `/subagent-driven-development` for review
8. **Mysterious bug, not obvious cause** → `/systematic-debugging` (4-phase: trace → defend → conditions → wait)
9. **Working on multiple features at once** → `/using-git-worktrees` for parallel branches
10. **After any fix** → `/verification-before-completion` to confirm it actually works

---

## What This Is

**Ha-Sketch** is a collection of 30 hand-drawn, sketchbook-style custom cards for Home Assistant dashboards. Inspired by [sketchbook-ui](https://github.com/SarthakRawat-1/sketchbook-ui).
...
| Plant | `sketch-plant-card` | Living plant illustration: species-accurate SVG (snake plant, ZZ, rubber), health-driven posture/drooping, sensor gauges with auto-discovered thresholds, status icons |
